...
void cgroup_append();
void cgroup_append_back(pid_t pid);
...
int main(int argc, char **argv) {
    ...
    char bind_path[PATH_SIZE] = {'\0'};
    read(carg.fds_c2p[0], bind_path, PATH_SIZE);
...
int do_pivot(const char *tmpdir) {
    ...
    snprintf(oldroot_path, sizeof(char)*PATH_SIZE, "%s/%s", tmpdir, put_old);
